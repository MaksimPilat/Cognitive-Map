export class Arrow{
    ...
    setCoord(node_1, node_2){
        ...
        if(node_2.x > node_1.x){
            this.begin.x = node_2.x - (this.length - node_1.size-10) * dx / this.length;   
        ...
        if(node_2.y > node_1.y){
            this.begin.y = node_2.y - (this.length - node_1.size-10) * dy / this.length;
    ...
    render(){
        if(this.length - this.beginNode.size - this.endNode.size > 20){
            ...
            if(this.isOriented){
                ...
                this.ctx.lineTo(this.end.x + Math.cos(alpha + 150/180*Math.PI)*20, this.end.y + Math.sin(alpha + 150/180*Math.PI)*20);  
                this.ctx.moveTo(this.end.x, this.end.y);
                this.ctx.lineTo(this.end.x + Math.cos(alpha - 150/180*Math.PI)*20, this.end.y + Math.sin(alpha - 150/180*Math.PI)*20);  
                this.ctx.stroke();
            }
            ...
            if(this.begin.x > this.end.x) x = this.begin.x - Math.abs(this.begin.x - this.end.x) / 2;
            else x = this.begin.x + Math.abs(this.begin.x - this.end.x) / 2;

            if(this.begin.y > this.end.y) y = this.begin.y - Math.abs(this.begin.y - this.end.y) / 2;
